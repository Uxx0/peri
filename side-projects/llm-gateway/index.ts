...
function writeReqFile(logDir: string, id: string, filename: string, content: string) {
  try {
...
function logRequest(
  ...
  if (logLevel === "body") {
    logC(90, "  HEADERS:", JSON.stringify(sanitizeHeaders(headers), null, 2));
...
function logResponse(
  ...
  if (logLevel === "body") {
    ...
    if (contentType.includes("json")) {
      logC(90, "  RESPONSE:", JSON.stringify(body, null, 2).slice(0, 4000));
    } else {
      logC(90, "  RESPONSE: (non-JSON, length=" + (body as string)?.length + ")");
    }
  }
  logSep();
}
...
export function resolveUrl(baseUrl: string, pathname: string, search: string): string {
...
export async function proxyRequest(
  ...
  if (!upstreamRes.body) {
    logResponse(config.logLevel, route, upstreamRes.status, latency, null, upstreamRes.headers);
    return new Response(null, { status: upstreamRes.status, headers: resHeaders });
  }

  const isJson = contentType.includes("json");
  const logFileName = isJson ? "response.json" : contentType.includes("event-stream") ? "stream.log" : "response.bin";
  ...
  const transform = new TransformStream({
    ...
    flush() {
      ...
      if (isJson) {
        try { resBody = JSON.parse(text); } catch { /* keep raw text */ }
      }

      writeReqFile(config.logDir, id, logFileName, isJson && typeof resBody !== "string" ? JSON.stringify(resBody, null, 2) : text);
      logResponse(config.logLevel, route, upstreamRes.status, latency, resBody, new Headers());
    },
  });

  upstreamRes.body.pipeTo(transform.writable).catch(() => {});
  return new Response(transform.readable, { status: upstreamRes.status, headers: resHeaders });
}
...
export function createHandler(config: GatewayConfig) {
  return function handler(req: Request): Response | Promise<Response> {
    ...
    if (
      ...
      return proxyRequest(config, "[openai]", config.openaiBase, pathname, req);
    }
    if (pathname.startsWith("/v1/messages")) {
      return proxyRequest(config, "[anthropic]", config.anthropicBase, pathname, req);
    }
    if (pathname === "/health") {
...
if (import.meta.main) {
  ...
  Bun.serve({ port: config.port, fetch: createHandler(config) });
  console.log(`LLM Gateway listening on http://localhost:${config.port}`);
  console.log();
  console.log("Routes:");
  console.log(`  POST /v1/chat/completions  → ${config.openaiBase}/v1/chat/completions`);
  console.log(`  POST /v1/responses         → ${config.openaiBase}/v1/responses`);
  console.log(`  GET  /v1/models            → ${config.openaiBase}/v1/models`);
  console.log(`  POST /v1/messages          → ${config.anthropicBase}/v1/messages`);
