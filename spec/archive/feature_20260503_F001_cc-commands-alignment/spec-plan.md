...
- [x] 创建 ConfigPanel 状态结构体
  ...
  - 实现 `from_config(cfg: &PeriConfig) -> Self`：从 AppConfig + CompactConfig 读取当前值填充缓冲区
  ...
  - 实现 `apply_edit(&mut self, cfg: &mut PeriConfig)`：将缓冲区值写回 PeriConfig
...
- [x] 为 ConfigPanel 状态逻辑编写单元测试
  ...
  - 测试场景:
    - `test_config_panel_from_config_defaults`: 从默认 PeriConfig 初始化，autocompact=true, threshold="85", language="", proactiveness="medium"
    ...
    - `test_config_panel_apply_edit_saves_to_config`: 编辑后 apply_edit 写入 PeriConfig，验证 cfg.config.language/persona/tone/proactiveness 正确
...
3. 验证 /config 面板完整流程
   ...
   - 检查 `~/.peri/settings.json` 中 `persona` 字段已更新
